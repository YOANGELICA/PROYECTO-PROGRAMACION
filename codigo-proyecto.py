...
# Cambiar por ('C:\Program Files (x86)\chromedriver.exe') de ser necesario, depende de la ruta del chromedriver.exe en cada pc
    ...
    for apt in apts:
        ...
        if arooms == h:
            with open(f'{"Informacion apartamentos"}/{count}.txt', 'w', encoding='utf-8') as f:
                ...
                f.write('\n\n')
                f.write(f"El costo de arriendo que le corresponde a cada una de las {h} personas es: ${arriendo} COP")
                count = count +1
